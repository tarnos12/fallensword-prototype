...
- **Modular files**, no monolith: `combat` `actors` `progression` `items` `quests` `techniques` `cards` `market` `personas` `guild` `boss` `tutorial` `loadouts` `map` `save` `game` `ui` `main` (+ `rng`).

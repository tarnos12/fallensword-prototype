...
2. **Onboarding/tutorial pass** + **visual/UI polish pass** (Stage 2 exit criteria: a stranger plays with zero explanation).
...
- **Modular files**, no monolith: `combat` `actors` `progression` `items` `quests` `techniques` `cards` `market` `personas` `guild` `map` `save` `game` `ui` `main` (+ `rng`).

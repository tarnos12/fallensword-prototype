...
**Resolved (2026-07-05):** every item is generated from a named **template** that defines an ordered list of attributes, each with a fixed roll range (e.g. Dagger: 1–2 Attack) scaled by item level. **Rarity gates the number of attributes rolled** — Common 1, Uncommon 2, Rare 3, Epic 4, Legendary/Mythic 5 — plus a range multiplier and durability/repair/sell tiers. **Item types are unique per rarity**: the name itself signals the tier (Bone-Carved Dagger is always Common, Serpent-Fang Dirk always Uncommon, Dragonvein Sabre always Epic), with no rarity prefixes. Random drops only reach Rare; Epic/Legendary/Mythic are hand-authored per the bullet above.

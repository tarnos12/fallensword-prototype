...
| B | **Gear set bonuses** (GDD §5) — item `setId` + N-piece set-bonus definitions that plug into the reserved `effectiveStats` hook. | `js/sets.js` *(new)* | `js/items.js` (add `setId` to some templates), `js/progression.js` (one add-set-bonuses line in `effectiveStats`), `js/ui.js` (tooltip shows set progress) | `DONE` | #2 | `claude/gear-sets` | #38 | 2026-07-07 |
| C | **Alchemy / consumables** (GDD §6.4) — brew pills from drops + stones: timed buff / instant Qi / instant XP, stored in a consumables pouch, used from the HUD. 🜁 modal. | `js/alchemy.js` *(new)* | `js/game.js` (wrappers + tick), `js/actors.js` (`player.consumables`), `index.html`/`css`/`js/main.js` | `DONE` | #3 | `claude/alchemy-consumables` | #30 | 2026-07-07 |
...
| H | **Core Formation realm + advanced techniques** (GDD §9.1) — add the 3rd realm to the ladder + tier-3/4 techniques for it. | `js/techniques.js` (additions) | `js/progression.js` (`REALMS` + `STAGE_XP`) — coordinate w/ F | `DONE` | #4 | `claude/core-formation-realm` | #34 | 2026-07-07 |
...
| M | **Salvage / materials** — right-click "salvage" breaks unwanted gear into crafting materials (feeds task A). Materials are a stackable item type on `player`. | `js/salvage.js` *(new)* | `js/items.js` (material item shape), `js/game.js` (salvage wrapper), `js/ui.js` (context-menu entry), `css` | `ASSIGNED` | #2 | `claude/salvage-materials` | — | 2026-07-07 |
...
| Q | **Sect disciple missions** — send hired disciples (task S sect) on timed wall-clock missions that return spirit stones / materials. Extends the Sect. | `js/sectmissions.js` *(new)* | `js/game.js` (tick + claim), `js/guild.js` (read members — no edit), `index.html`/`css`/`main.js` | `DONE` | #4 | `claude/sect-missions` | #31 | 2026-07-07 |
...
| U | **Gem sockets / enchanting** — sockets on higher-rarity gear; slot gems (a dropped item type) for flat bonuses that flow through `effectiveStats`. | `js/sockets.js` *(new)* | `js/items.js` (`sockets` on templates + gem item type), `js/progression.js` (add gem bonuses in `effectiveStats`), `js/ui.js` (tooltip), `css` | `DONE` | #2 | `claude/gem-sockets` | #33 | 2026-07-07 |
| V | **Ascension / New Game+** — at max realm, reset progression for a permanent "ascension" multiplier (kept across resets). A prestige loop for replay. | `js/ascension.js` *(new)* | `js/game.js` (reset-with-keep flow), `js/progression.js` (apply the multiplier), `js/actors.js` (`player.ascension`), `index.html`/`css`/`main.js` | `DONE` | #4 | `claude/ascension-ng-plus` | #37 | 2026-07-07 |
...
- 2026-07-07 — session_01Sty (#1, manager) — merged + reviewed 5 more PRs: #35 (E world-data refactor), #34 (H Core Formation realm), #38 (B gear sets), #37 (V ascension), #36 (AB nav/HUD redesign). Ordered the merges E→H→B→V→AB to sequence the shared `progression.js` `effectiveStats` edits (B = 6th flat source; V = final ascension scalar — verified applied after the flat sources) and land the structural HUD redesign last. AB verified: Qi/XP meters fill, every binding id preserved (`#chip-qi` tutorial-safe), 15 nav buttons, 0 console errors. **Master Stage-3 merged: 25 tasks.** **Board nearly drained** — only F (#3, unblocked by E), M, J remain. Refilled: #2 → **M** (salvage); #4 → **J** (balance pass + sim harness, now unblocked since content is essentially complete); #3 advances to **F** (third zone). **After F/M/J land, Stage-3 content is complete → merge the held strip-testing #13 for 1.0.** Flagged to workers via Inbox.
- 2026-07-07 — session_01Sty (#1, manager) — added the **collision-free Inbox/Outbox messaging protocol** (star topology through #1). Then merged + reviewed 4 more worker PRs: #30 (C alchemy), #31 (Q sect missions), #32 (Z mobile/responsive), #33 (U gem sockets). **Review notes:** #30's pill combat-buffs deliberately bypass `player.activeBuffs`/`effectiveStats` (which `ui.js` renders via a throwing technique lookup) and apply at combat time via `applyPillBuffs` — sound isolation; #33 auto-merged cleanly with Y's `compareRows` (both `socketLine`+`compareRows` in the tooltip, `socketBonuses` the 5th `effectiveStats` flat source). All verified real Chromium (14 nav buttons, 8 stylesheets, 0 console errors). C/Q/Z/U → DONE. **Master Stage-3 merged: 20 tasks** (A/G/P/D/O/I/N/W/X/T/AA/L/S3/R/Y/K/C/Q/Z/U). **Refilled all three queues** (2 tasks each): #2 → AB (nav/HUD, now unblocked since W–AA landed) + B (sets); #3 → E (world-data refactor) + F (third zone, after E); #4 → H (Core Formation realm) + V (ascension). **Cross-coordination:** F (#3) & H (#4) both touch the `progression.js` realm ladder — noted in both Inboxes.
